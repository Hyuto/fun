...
if __name__ == "__main__":
    ...
    yolov5_seg = ort.InferenceSession("./models/yolov5s-seg.onnx")
    nms = ort.InferenceSession("./models/nms-yolov5.onnx")
    ...
    output = yolov5_seg.run(None, {"images": input_})
    test_output = nms.run(None, {"detection": output[0], "config": nms_config})

    selected = output[0][:, test_output[0][0], :]

    all_mask = np.zeros([max_size, max_size, 4], dtype=np.uint8)

    for i in range(selected.shape[1]):
        box = selected[0, i, :4]
        confidence = selected[0, i, 4]
        score = confidence.reshape(-1, 1) * selected[0, i, 5:85]
        masked = selected[0, i, 85:]
    ...
    cv2.imwrite(f"output/val.png", all_mask)
    onnx.save(model, "models/mask-yolov5-seg.onnx")  # saving onnx model
